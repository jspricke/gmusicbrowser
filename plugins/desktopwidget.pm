...
{	my $vbox=Gtk3::VBox->new(::FALSE, 2);
	...
	my $layoutlabel= Gtk3::Label->new(_"Layout :");
	my $add= ::NewIconButton('gtk-add',_"Add", sub { New($LayoutCombo->get_value); });

	my $store=Gtk3::ListStore->new('Glib::String','Glib::Boolean','Glib::String');
	$Treeview=Gtk3::TreeView->new($store);
	...
	my $togglerenderer=Gtk3::CellRendererToggle->new;
	$togglerenderer->signal_connect(toggled => \&toggled);
	$Treeview->append_column( Gtk3::TreeViewColumn->new_with_attributes( 'active', $togglerenderer, active => 1 ));
	my $renderer=Gtk3::CellRendererText->new;
	$Treeview->append_column( Gtk3::TreeViewColumn->new_with_attributes( 'name', $renderer, text => 2 ));
	...
	$OptionsBox=Gtk3::VBox->new(::FALSE, 2);
...
{	my $key=shift;
	...
	my $textcolor= Gtk3::ColorButton->new_with_color( Gtk3::Gdk::Color->parse($opt->{DefaultFontColor}) );
	...
	my $textfont= Gtk3::FontButton->new_with_font( $opt->{DefaultFont} );
	$textfont->signal_connect(font_set=>sub { $opt->{DefaultFont}= $_[0]->get_font_name; CreateWindow($key); });

	my $adjx=Gtk3::Adjustment->new($opt->{x},0,100,1,10,0);
	my $adjy=Gtk3::Adjustment->new($opt->{y},0,100,1,10,0);
	#my $spinx=Gtk3::SpinButton->new($adjx,0,0);
	#my $spiny=Gtk3::SpinButton->new($adjy,0,0);
	my $spinx=Gtk3::HScale->new($adjx);
	my $spiny=Gtk3::HScale->new($adjy);
	$spinx->set_digits(0);
	$spiny->set_digits(0);
	...
	my $adj_mon=Gtk3::Adjustment->new($opt->{monitor},0,$monitors-1,1,2,0);
	my $spin_mon= Gtk3::SpinButton->new($adj_mon,0,0);
	$spin_mon->set_sensitive(0) if $monitors<2;
	$adj_mon->signal_connect(value_changed => sub { $opt->{monitor}=$_[0]->get_value; MoveWindow($key);  });

	my $adjo=Gtk3::Adjustment->new($opt->{opacity}*100,0,100,1,10,0);
	my $spino=Gtk3::SpinButton->new($adjo,0,0);
	...
	my $ontop=Gtk3::CheckButton->new(_"On top of other windows instead of below");
	...
	my $adjw=Gtk3::Adjustment->new($opt->{w},1,::max(2000,$opt->{w},Gtk3::Gdk::Screen->get_default->get_width),10,50,0);
	my $adjh=Gtk3::Adjustment->new($opt->{h},1,::max(2000,$opt->{h},Gtk3::Gdk::Screen->get_default->get_height),10,50,0);
	my $spinw=Gtk3::SpinButton->new($adjw,0,0);
	my $spinh=Gtk3::SpinButton->new($adjh,0,0);
	...
	my $vbox= ::Vpack(
		[ $label, '-',$remove ],
		[ Gtk3::Label->new(_"Default text color"), $textcolor],
		[ Gtk3::Label->new(_"Default text font"),  $textfont],
		[ Gtk3::Label->new(_"Centered on"),  '_',$spinx, Gtk3::Label->new('%  x'), '_',$spiny, Gtk3::Label->new('%') ],
		[ Gtk3::Label->new(_"Monitor"), $spin_mon, ],
		[ Gtk3::Label->new(_"Minimum size"), $spinw, Gtk3::Label->new('x'), $spinh ],
		[ Gtk3::Label->new(_"Opacity"),  $spino, Gtk3::Label->new('%') ],
		 $ontop);
	$OptionsBox->pack_start($vbox,::FALSE,::FALSE,2);
	$OptionsBox->show_all;
}
